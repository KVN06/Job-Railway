...
<head>
    ...
    <title>Iniciar sesión | Job Opportunity</title>
...
<body class="relative min-h-screen bg-neutral-950 text-neutral-50">
    ...
    <header class="relative z-10 border-b border-white/10 bg-neutral-950/70 backdrop-blur">
        <div class="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-5 fade-in-up">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('login') }}" class="hidden items-center gap-2 rounded-full bg-sky-500 px-4 py-2 text-sm font-semibold text-white shadow-lg transition hover:bg-sky-400 md:inline-flex">
                    <i class="fas fa-arrow-right-to-bracket"></i>
                    Iniciar sesión
                </a>
                <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 text-sm font-semibold text-white transition hover:border-white hover:bg-white/10">
    ...
    <main class="relative z-10 px-4 py-16 sm:px-6 lg:px-8">
        <section class="mx-auto flex w-full max-w-6xl flex-col gap-12">
            @if ($errors->any())
                <div class="mx-auto w-full max-w-3xl rounded-2xl border border-red-400/40 bg-red-500/10 p-5 text-red-100 backdrop-blur">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <p class="text-base font-semibold">No pudimos iniciar tu sesión:</p>
            ...
            <div class="relative grid w-full overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-2xl backdrop-blur-xl transition hover:border-white/20 hover:shadow-3xl lg:grid-cols-2 fade-in-up" style="--fade-delay: 0.2s;">
                <aside class="floating-card flex flex-col justify-center bg-neutral-950/60 p-10 text-neutral-100">
                    <span class="inline-flex w-fit items-center gap-2 rounded-full bg-white/5 px-4 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-sky-200">Tu portal de empleo</span>
                    <h2 class="mt-6 text-3xl font-bold sm:text-4xl">Conecta con oportunidades reales</h2>
                    <p class="mt-4 text-base leading-relaxed text-neutral-200/80">Gestiona postulaciones, recibe alertas personalizadas y muestra tu perfil profesional a empresas que buscan talento como el tuyo.</p>

                    <ul class="mt-8 space-y-4 text-sm font-medium text-neutral-100">
                        <li class="flex items-center gap-3"><i class="fas fa-check text-sky-300"></i> Seguimiento en tiempo real a tus postulaciones</li>
                        <li class="flex items-center gap-3"><i class="fas fa-check text-sky-300"></i> Recomendaciones basadas en tu perfil</li>
                        <li class="flex items-center gap-3"><i class="fas fa-check text-sky-300"></i> Recursos para impulsar tu próxima entrevista</li>
                    </ul>
                ...
                <div class="relative glass-card p-10 text-neutral-900">
                    ...
                    <div class="mb-8 flex flex-col gap-3">
                        ...
                        <h1 class="text-3xl font-bold text-neutral-900 sm:text-4xl">Bienvenido de vuelta</h1>
   ...
   class="group flex items-center justify-center gap-3 rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-neutral-700 shadow-sm transition hover:-translate-y-0.5 hover:border-neutral-300 hover:bg-neutral-50">
    ...
    Iniciar sesión con Google
                    ...
                    <form action="{{ route('inicia-sesion') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-medium text-neutral-700">Contraseña</span>
                                <a href="{{ route('password.request') }}" class="font-medium text-sky-600 transition hover:text-sky-500">¿Olvidaste tu contraseña?</a>
                            </div>
                            <div class="relative mt-2">
                                    <input type="password" id="passwordInput" name="password" autocomplete="current-password" placeholder="••••••••" required class="form-input w-full rounded-2xl border border-neutral-300/80 bg-white/90 px-4 py-3 pr-14 text-sm font-medium text-neutral-800 shadow-sm transition focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-100">
                    ...
                    <p class="mt-6 text-center text-sm text-neutral-600">¿No tienes una cuenta?
                        <a href="{{ route('register') }}" class="font-semibold text-sky-600 transition hover:text-sky-500">Regístrate</a>
                    </p>
                </div>
            </div>

            <p class="mx-auto max-w-3xl text-center text-xs text-neutral-200/80 fade-in-up" style="--fade-delay: 0.35s;">Al continuar aceptas nuestros <a href="#" class="font-semibold text-sky-200 hover:text-sky-100">Términos de servicio</a> y <a href="#" class="font-semibold text-sky-200 hover:text-sky-100">Aviso de privacidad</a>.</p>
        </section>
    </main>
    ...
    <footer class="relative z-10 border-t border-white/10 bg-neutral-950/80">
        <div class="mx-auto flex w-full max-w-4xl flex-col items-center gap-3 px-6 py-6 text-sm text-neutral-300">
            ...
            <p class="text-center text-neutral-400">Tu espacio seguro para encontrar o publicar oportunidades laborales.</p>
